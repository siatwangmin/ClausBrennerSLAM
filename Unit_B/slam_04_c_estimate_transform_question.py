...
def find_cylinder_pairs(cylinders, reference_cylinders, max_radius):
    ...
    for i in xrange(len(cylinders)):
        ...
        for j in xrange(len(reference_cylinders)):
            point_r = reference_cylinders[j];
            dist = sqrt((point_c[0] - point_r[0]) * (point_c[0] - point_r[0]) +
                   (point_c[1] - point_r[1]) * (point_c[1] - point_r[1]))
            if dist < max_radius:
                indx = j
        if indx != -1:
            cylinder_pairs.append((i, indx))
...
def estimate_transform(left_list, right_list, fix_scale = False):
    ...
    left_list_prime = []
    right_list_prime = []
    for left_point in left_list:
        left_list_prime.append(((left_point[0] - lc[0]), left_point[1] - lc[1]))
    for right_point in right_list:
        right_list_prime.append(((right_point[0] - rc[0]), (right_point[1] - rc[1])))

    cs = 0.0
    ss = 0.0
    rr = 0.0
    ll = 0.0
    for i in xrange(len(left_list_prime)):
        rx = right_list_prime[i][0]
        ry = right_list_prime[i][1]
        lx = left_list_prime[i][0]
        ly = left_list_prime[i][1]
        cs += rx * lx + ry * ly
        ss += - rx*ly + ry * lx
        rr += rx * rx + ry * ry
        ll += lx * lx + ly * ly
